...
export const LibraryProvider = ({ children }: { children: ReactNode }) => {
  const { accessToken, refreshToken, tokenExpiry, ensureValidToken, logout } =
    useAuth();
  ...
  >(null);
  const [savedEpisodesNextUrl, setSavedEpisodesNextUrl] = useState<
    string | null
  >(null);
  ...
  const [isLoadingMoreSavedTracks, setIsLoadingMoreSavedTracks] =
    useState(false);
  const [isLoadingMoreSavedEpisodes, setIsLoadingMoreSavedEpisodes] =
    useState(false);
  ...
  const [isRefreshingSavedEpisodes, setIsRefreshingSavedEpisodes] =
    useState(false);
  ...
  const fetchInitialData = useCallback(
    async (token: string) => {
      if (isFetchingInitialData) {
        logInfo(
          "LibraryContext: Initial data fetch already in progress, skipping..."
        );
        return;
      }
  ...
  const fetchSavedEpisodes = useCallback(async () => {
    ...
    const result = await fetchSavedEpisodesService(
      accessToken,
      makeApiRequestWithContext,
      saveCachedData
    );
  ...
  useEffect(() => {
    const triggerInitialDataFetch = async () => {
      if (accessToken && !initialDataFetchTriggered && isOnline) {
        ...
        logInfo(
          "LibraryContext: Auth state loaded and online, proceeding with data fetch..."
        );
        ...
        logInfo(
          "LibraryContext: Offline - skipping initial data fetch, using cached data"
        );
        setInitialDataFetchTriggered(true);
      }
    };
  ...
  const value: LibraryContextType = useMemo(
    ...
    [
      ...
      fetchSavedTracks,
      fetchMoreSavedTracks,
      refreshSavedAlbumsFromCacheMethod,
      refreshFollowedPodcastsFromCacheMethod,
      refreshFollowedArtistsFromCacheMethod,
      savedEpisodes,
      savedEpisodesNextUrl,
      isLoadingMoreSavedEpisodes,
      isRefreshingSavedEpisodes,
      fetchSavedEpisodes,
      fetchMoreSavedEpisodes,
      refreshSavedEpisodesFromCacheMethod,
      clearCachedData,
      makeApiRequestWithContext,
      addTrackToPlaylist,
    ]
