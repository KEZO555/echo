...
export const makeApiRequest = async (
    ...
    if (isExpiredOrExpiringSoon) {
      ...
      try {
        ...
        if (refreshed) {
          ...
          logWarn("API: Token refresh failed, proceeding with current token");
        }
      } catch (error) {
        logError(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      logError(
        `Error fetching ${errorMessage.toLowerCase()}: ${response.status}`,
        errorData
      );
      if (response.status === 401 && retryCount < 1 && refreshToken) {
        logWarn("Token might be expired. Attempting to refresh token.", {
          url,
        ...
        if (refreshed) {
          ...
          // Get the updated token from secure storage after refresh
          const updatedToken = await SecureStore.getItemAsync(AUTH_TOKEN_KEY);
          log("API: Using updated token for retry after 401 refresh", {
            hasUpdatedToken: !!updatedToken,
          });
          return makeApiRequest(
            url,
